...
The primary goal of CMP package is to port the MHM and CMP softwares by Gaucherel and colleagues to R. They were originally written in Java and are no longer maintained (but see [MHM](http://amap-dev.cirad.fr/projects/mhm) and [CMP](http://amap-dev.cirad.fr/projects/cmp) legacy pages).
...
```{r}
p(l_mhm, "Monoscale maps (contagion)")
```

Finally, we calculate and show a multiscale map and a profile plot:

```{r}
app(l_mhm, mean) %>% p("Multiscale map (averaged contagion)")
ms_profile(l_mhm)
```
...
# plots now
l12_cmp %>% p("Monoscale maps (euclidean distance)")  # monoscale maps
l12_cmp %>% app(mean) %>% p("Multiscale maps (averaged)) # multiscale map
